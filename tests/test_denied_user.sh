...
echo "some_other_user" | sudo tee /etc/myRPC/users.conf >/dev/null
...
set +e
OUTPUT="$(./bin/myRPC-client -h 127.0.0.1 -p 1234 -s -c "whoami")"
CODE="$?"
set -e

echo "$OUTPUT" | grep "server response code: 1"
echo "$OUTPUT" | grep "user is not allowed"
